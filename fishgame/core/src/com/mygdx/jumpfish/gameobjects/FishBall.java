...
public class FishBall extends AbstractGameObject {
...
//	public Box2DSprite box2DSprite;
	...
	float cos1;
	float sin1;
	public FishBall(Body b) {
				body = b;
		init();
	}
	public FishBall(Body b,Body bb,float cos,float sin) {
		emitboy=bb;
		body = b;
		cos1 = cos;
		sin1 = sin;
init();
	...
	//	body.setUserData("fdsfsd");
		body.getFixtureList().get(0).setUserData("fish_ball");
		
		animatedSprite = new AnimatedSprite(AssetsMy.instance.fish.fish_vistrel);
		//	animatedSprite.pause();
			animatedBox2DSprite = new AnimatedBox2DSprite(animatedSprite);
	...
	//	body.setGravityScale(0);
		...
		//body.setLinearVelocity(1,1);
			...
			body.setTransform(emitboy.getPosition().x,emitboy.getPosition().y,0);
		//	body.setLinearVelocity(new Vector2(-(float)MathUtils.cos(180-ugol1)*4,(float)MathUtils.sin(180-ugol1)*4));	
			body.applyLinearImpulse(new Vector2(cos1*5,sin1*5),body.getPosition(),true);
	...
	//	System.out.println(body.getFixtureList().get(0).getUserData());
		...
		if (body.getFixtureList().get(0).getUserData().equals("PreParingBAM"))
		{
			
			body.setActive(false);	body.getFixtureList().get(0).setUserData("BAM");
		}
		}
