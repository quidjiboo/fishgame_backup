...
public class CreditsLevelMy {
	public float skor=0.01f;
...
//	public Array<Ball> bools;
	...
	public KnopkaNotActiv knopkatext;
	public KnopkaNotActiv knopkaverx;
	public KnopkaNotActiv knopkavniz;
	...
	public static final String TAG = CreditsLevelMy.class.getName();
	...
	map = loader.load("maps/creditsmap.tmx");
	...
	//System.out.println(MY_f_world.getBodyCount());
		for (int i = 0; i < MY_f_world.getBodyCount();i++){
			...
			if (testbody.getUserData().equals("portal_body")){
				...
				if (testbody.getUserData().toString().contains("button_text")){
					System.out.println("credits");
					
					knopkatext = new KnopkaNotActiv(bodies1.get(i));
					
					}
				if (testbody.getUserData().toString().contains("button_verx")){
					System.out.println("credits");
					
					knopkaverx = new KnopkaNotActiv(bodies1.get(i));
					
					}
				if (testbody.getUserData().toString().contains("button_vniz")){
					System.out.println("credits");
					
					knopkavniz = new KnopkaNotActiv(bodies1.get(i));
	...
	public void update (float deltaTime) {
			if( knopkatext.getBody().getPosition().y<30)
		knopkatext.getBody().setTransform(knopkatext.getBody().getPosition().x, knopkatext.getBody().getPosition().y+skor, 0);
