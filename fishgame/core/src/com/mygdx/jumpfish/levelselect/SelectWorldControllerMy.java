...
public class SelectWorldControllerMy extends InputAdapter implements Disposable {
	public	float timerok=0; 
	public MenuGUI Mygui;
	private static final String TAG = SelectWorldControllerMy.class.getName();
	private boolean below = false;
	private DirectedGameMy game;
	 public SelectLevelMy level; 
...
//	 static public float lives; //переделать и отдать в свойство рфбки
	...
	public SelectWorldControllerMy(DirectedGameMy game2) {
...
//		LogikaObektov(deltaTime);// тут 
	...
	//		}
		//	ChangeViw(); //ЗАКОМЕНТИЛ!
			...
			cameraHelper.setPosition(26f ,7.5f);
	...
	private void ChangeNavigation(int x, int y) {
		if (SelectWorldRendererMy.x1 < SelectWorldRendererMy.x2	&& level.getPlayer().PreperingTime > 20&& SelectWorldRendererMy.y2 > SelectWorldRendererMy.y1) {
			level.getPlayer().body.applyLinearImpulse(new Vector2(level.getPlayer().PreperingTime * 0.1f* SelectWorldRendererMy.cos,level.getPlayer().PreperingTime * 0.1f* SelectWorldRendererMy.sin),level.getPlayer().body.getPosition(), true);
		...
		if (SelectWorldRendererMy.x1 < SelectWorldRendererMy.x2&& level.getPlayer().PreperingTime < 20) {
	...
	private void ChangeNavigationsmash(int x, int y) {
		...
		if (SelectWorldRendererMy.x1 < SelectWorldRendererMy.x2) {
			level.getPlayer().body.applyLinearImpulse(new Vector2(2.5f, 1f),new Vector2(0f, 0f), true);
			level.getPlayer().whilbeslap = false;
		} else 
		if (SelectWorldRendererMy.x1 > SelectWorldRendererMy.x2) {
...
game.setScreen(new GameScreenMy(game,Start),transition);}
	...
	//	System.out.println("!!!!"+x/MenuWorldRendererMy.UnitScale);
		...
		for (Knopka pl : level.kopki)
		{
			
		ChangeLevel(pl.PressKnopka(x, y));}
