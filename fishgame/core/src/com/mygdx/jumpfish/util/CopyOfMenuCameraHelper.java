...
public class CopyOfMenuCameraHelper {

	private static final String TAG = CopyOfMenuCameraHelper.class.getName();
	Vector2 lerpTarget = new Vector2();
	private final float MAX_ZOOM_IN = 2.13f;
	private final float MAX_ZOOM_OUT1 = 2.13f;
...
//	private float viewportHeight1;
	...
	public void update (float deltaTime) {
		...
		position.x = Math.max(-MenuWorldRendererMy.XXX+0.1f,position.x);
		position.y = target.position.y;
		position.y = Math.max(-MenuWorldRendererMy.YYY+0.1f,position.y);
	...
	//		position.y = 24.75f+WorldRendererMy.YYY+0.1f;
		if(position.x>HelperX+MenuWorldRendererMy.XXX+0.1f)
			position.x = HelperX+MenuWorldRendererMy.XXX+0.1f;
		if(position.y>HelperY+MenuWorldRendererMy.YYY+0.1f)
			position.y =HelperY+MenuWorldRendererMy.YYY+0.1f;
